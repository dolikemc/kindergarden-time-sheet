...
class DateHandler:
    ...
    def __init__(self, sheet: Union[Worksheet, None], config: Configurator) -> None:
        ...
        self.year = self._config.get('year', datetime.today().year)
        ...
        self.date_format = self._config.get('format', '%d/%m')
        delay_value = self._config.get('update_delay', 0)
        if delay_value > 0:
            self.update_delay = f"-{delay_value}"
        else:
            self.update_delay = ""
        self.overtime_factor = self._config.get('overtime_factor', 1.5)
        self.styles: List = self._config.get('styles', ['Output', 'Output', 'Output'])

        self._special_days = {}
        for name in self._config.get('holidays', [{'name': ''}]):
            for date_record in name.get('dates', []):
                month_day = datetime.strptime(date_record, self.date_format)
                key_date = date(year=self.year, month=month_day.month, day=month_day.day)
    ...
    def year_iterator(self) -> Iterator[DataRow]:
        ...
        return_row = DataRow()
        for i in range(0, (date(year=self.year + 1, month=1, day=1) - date(year=self.year, month=1, day=1)).days):
            if day in self._holidays:
                return_row.date, return_row.type, return_row.name = day, 2, self._config.get('holiday', 'Holiday')
            elif day.weekday() > 4:
                return_row.date, return_row.type, return_row.name = day, 1, ''
            elif day.isoformat() in self._special_days.keys():
                return_row.date, return_row.type, return_row.name = day, 3, self._special_days[day.isoformat()]
            else:
                return_row.date, return_row.type = day, 0
            yield return_row
            day += timedelta(days=1)

    def add_row(self, hours: List, stop: str = '', start: str = '') -> int:
        if len(hours) < 5:
            raise Exception('you have to provide hours for each weekday')

        dv = self.create_header()
        start_day = self.determine_stop_date(start)
        stop_date = self.determine_stop_date(stop)
        for index, day_row in enumerate(self.year_iterator()):
            # stop criteria for a member
            if day_row.date > stop_date:
                return 1

            if day_row.type in (1, 2, 3):
                for c in range(1, 8):
                    self._worksheet.cell(row=index + 2, column=c).style = self.styles[day_row.type - 1]
            ...
            if day_row.type in (0, 3):
                ...
                if day_row.date < start_day:
                    self._worksheet.cell(row=index + 2, column=3, value="00:00")
                    self._worksheet.cell(row=index + 2, column=5, value="0")
                    self._worksheet.cell(row=index + 2, column=17, value="0")
                else:
                    ...
                    self._worksheet.cell(
                        ...
                        value=f'=IF(AND(A{index + 2}<TODAY(){self.update_delay},C{index + 2}<>"",F{index + 2}=""),'
                              f'IF(C{index + 2}=0,D{index + 2}*{self.overtime_factor},'
    ...
    def determine_stop_date(self, stop):
        if stop:
            month_day = datetime.strptime(stop, self.date_format)
        else:
            month_day = date(year=self.year, month=12, day=31)
        return date(year=self.year, month=month_day.month, day=month_day.day)
    ...
    def summary(self, name: str):
        ...
        # overtime cells
        # =TEXT(ABS(SUMME((-1+2*(LINKS(G11:G22)<>"-"))*(RECHTS(0&G11:G22;5))));WENN(SUMME((-1+2*(LINKS(G11:G22)<>"-"))
        # *(RECHTS(0&G11:G22;5)))<0;"-";)&"[hh]:mm:ss")
        ...
        self.set_cell_std_format(from_row=15, from_column=8, style_number=3, text=f"Soll {self.year}")
        ...
        self.set_cell_std_format(from_row=15, from_column=12, style_number=3, text=f"{self.year}")
        ...
        self.set_cell_std_format(from_row=15, from_column=14, style_number=3, text=f"Soll {self.year}")
    ...
    def set_cell_std_format(self, from_row: int, from_column: int, style_number: int = None, text: str = None,
        ...
        if style_number:
            cell.style = self.styles[style_number]
        if text:
            cell.value = text
